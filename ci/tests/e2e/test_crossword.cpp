  ...
  <script language="lua">
    ...
    local function colorCell(r, c)
      ...
      if solvedCount == 0 then
        ui.setAttr(cid(r,c), "bgcolor", CELL)
      elseif solvedCount < totalCount then
        ui.setAttr(cid(r,c), "bgcolor", PARTIAL)  -- пересечение, частично
      else
        ui.setAttr(cid(r,c), "bgcolor", CORRECT)  -- полностью
      end
    end
    ...
    local function highlight()
      ...
      if not solved[curWord] then
        ...
        ui.setAttr(cid(r,c), "bgcolor", SELECTED)
      end
    end
      ...
      for r = 0, 4 do
        for c = 0, 4 do
          if activeCell[r .. "," .. c] then
            ...
            ui.setAttr(cid(r,c), "bgcolor", BLACK)
          end
        end
      end
    end
