...
static void setupVFS() {
    ...
    LittleFS.writeFile("/apps/calc/calc.bax",
    ...
    LittleFS.mkdir("/apps/hello");
    LittleFS.writeFile("/apps/hello/hello.bax",
        "<app>\n"
        "  <ui default=\"/main\">\n"
    ...
    LittleFS.writeFile("/apps/game/game.bax",
    ...
    LittleFS.writeFile("/apps/game/icon.png", "PNGDATA");
...
int main() {
    ...
    TEST("config loads from VFS file") {
        LittleFS.reset();
        setupVFS();
        LittleFS.writeFile("/system/config.yml",
            "display:\n"
            "  brightness: 100\n"
            "power:\n"
            "  auto_sleep: false\n"
    ...
    TEST("refreshApps rescans and picks up new app") {
        ...
        LittleFS.mkdir("/apps/newapp");
        LittleFS.writeFile("/apps/newapp/newapp.bax",
