...
int main() {
    ...
    TEST("load partial — undefined keys ignored") {
        LittleFS.reset();
        LittleFS.writeFile("/partial.yml",
            "display:\n"
            "  brightness: 100\n"
            "  unknown_key: 999\n"
        );
    ...
    TEST("load with comments") {
        LittleFS.reset();
        LittleFS.writeFile("/comments.yml",
            "# This is a comment\n"
            "display:\n"
    ...
    TEST("save creates file in VFS") {
        ...
        cfg.save();
        if (LittleFS.exists("/out.yml")) PASS(); else FAIL("file not created");
    }
    ...
    TEST("save creates parent directory") {
        ...
        cfg.save();
        if (LittleFS.exists("/system")) PASS(); else FAIL("parent not created");
    }
