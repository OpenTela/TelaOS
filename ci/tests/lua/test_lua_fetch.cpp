/**
 * Test: Lua Fetch
 * fetch() registration, BLE not connected handling, net.connected() function
 */
#include <cstdio>
#include <cstring>
#include <string>
...
int main() {
    ...
    TEST("net.connected is function") {
        int err = luaL_dostring(L, "assert(type(net.connected) == 'function')");
        if (err == 0) PASS(); else { FAIL(lua_tostring(L, -1)); lua_settop(L, 0); }
    }
    ...
    TEST("net.connected() returns false in mock") {
        int err = luaL_dostring(L, "assert(net.connected() == false)");
        if (err == 0) PASS(); else { FAIL(lua_tostring(L, -1)); lua_settop(L, 0); }
    }
